...
> **Source:** [`terraform/modules/diagram/`](terraform/modules/diagram/) — `assets/ck1m.drawio` and `assets/ck1m.svg` are generated by `terraform apply` via the `diagram` module. Open `ck1m.drawio` in [draw.io](https://app.diagrams.net) or the VS Code Draw.io extension to inspect the diagram interactively.
